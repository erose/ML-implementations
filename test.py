...
class TestModel(unittest.TestCase):
  def test_model_can_be_printed(self):
    model = lr.LinearModel(np.array([[2, 1, 0]]).T)
...
class TestLinearRegression(unittest.TestCase):
  def test_cost_function_on_constant_models(self):
    ...
    dumb_model = lr.LinearModel(np.array([[6, 0]]).T)
  ...
  def test_cost_function(self):
    ...
    model = lr.LinearModel(np.array([[6, 1]]).T)
    ...
    correct_model = lr.LinearModel(np.array([[0, 2]]).T)
